...
export default factories.createCoreController(
  ...
  ({ strapi }: { strapi: Strapi }) => ({
    ...
    // Test SnappPay Cancel (after settlement)
    async testSnappayCancel(ctx) {
      try {
        const { paymentToken } = ctx.request.body || {};

        if (!paymentToken) {
          return ctx.badRequest("paymentToken is required", {
            data: { success: false, error: "Missing paymentToken" },
          });
        }

        strapi.log.info("Testing SnappPay Cancel", { paymentToken });

        const snappay = strapi.service("api::payment-gateway.snappay");
        const result = await snappay.cancel(paymentToken);

        strapi.log.info("SnappPay Cancel Result", result);
        ...
        strapi.log.error("SnappPay Cancel Test Error:", error);
        return ctx.badRequest("Cancel test failed", {
          data: { success: false, error: error.message },
        });
      }
    },

    // Test SnappPay Revert (before settlement)
    async testSnappayRevert(ctx) {
      try {
        const { paymentToken } = ctx.request.body || {};
        ...
        strapi.log.info("Testing SnappPay Revert", { paymentToken });

        const snappay = strapi.service("api::payment-gateway.snappay");
        const result = await snappay.revert(paymentToken);

        strapi.log.info("SnappPay Revert Result", result);
        ...
        strapi.log.error("SnappPay Revert Test Error:", error);
        return ctx.badRequest("Revert test failed", {
          data: { success: false, error: error.message },
        });
      }
    },

    // Test SnappPay Status inquiry
    async testSnappayStatus(ctx) {
      try {
        const { paymentToken } = ctx.request.body || {};
        ...
        strapi.log.info("Testing SnappPay Status", { paymentToken });

        const snappay = strapi.service("api::payment-gateway.snappay");
        const result = await snappay.status(paymentToken);

        strapi.log.info("SnappPay Status Result", result);
        ...
        strapi.log.error("SnappPay Status Test Error:", error);
        return ctx.badRequest("Status test failed", {
          data: { success: false, error: error.message },
        });
      }
    },

    // Test SnappPay Update (after settlement, reduce amount)
    async testSnappayUpdate(ctx) {
      try {
        const { paymentToken, updatedCart } = ctx.request.body || {};

        if (!paymentToken || !updatedCart) {
          return ctx.badRequest("paymentToken and updatedCart are required", {
            data: { success: false, error: "Missing paymentToken or updatedCart" },
          });
        }

        strapi.log.info("Testing SnappPay Update", {
          paymentToken,
          newAmount: updatedCart.amount,
        });

        const snappay = strapi.service("api::payment-gateway.snappay");
        const result = await snappay.update(paymentToken, updatedCart);

        strapi.log.info("SnappPay Update Result", result);
        ...
        strapi.log.error("SnappPay Update Test Error:", error);
