...
#### For `api.infinitycolor.co` (Backend API)
```
Type: A or CNAME
...
#### Payment Gateways
- **Mellat Bank**: Update callback URL to `https://api.infinitycolor.co/api/orders/payment-callback`
- **SnappPay**: Update return URL to `https://api.infinitycolor.co/api/orders/payment-callback`
...
**Solution**: Verify `NEXT_PUBLIC_IMAGE_BASE_URL` is set to `https://api.infinitycolor.co/` in frontend environment.
...
**Solution**: Verify `NEXT_PUBLIC_API_BASE_URL` is set to `https://api.infinitycolor.co/api` in frontend environment.
