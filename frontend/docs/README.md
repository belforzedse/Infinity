# Documentation Index

Project documentation and API references for Infinity Store frontend.

## Frontend Docs

- Developer guide (root): `../DEVELOPMENT.md`
- Cart API guide: `docs/cart-api-guide.md`
- Postman collection: `docs/cart-api-postman-collection.json`

## Notes

- The frontend integrates with a Strapi v4 backend. See environment variables in the project root `README.md` and `DEVELOPMENT.md` for configuration.
- Dockerfiles expect required environment variables to be supplied at build time. See root docs for configuration.

Inline code is documented with JSDoc comments to aid contributors in
understanding utilities and components. See the project root README for
additional pointers on comment conventions.

