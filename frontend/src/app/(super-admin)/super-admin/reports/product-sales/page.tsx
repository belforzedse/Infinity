...
import { Tooltip as RechartsTooltip, ResponsiveContainer, Cell, Treemap } from 'recharts';
        ...
        <div className="bg-white rounded-2xl p-5">
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl p-6 border border-blue-100">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-lg font-medium text-neutral-700 mb-2">مجموع فروش</h3>
                ...
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-lg font-medium text-neutral-700 mb-2">تعداد فروش</h3>
              ...
              {/* Chart Section */}
              <div>
                <h3 className="text-lg font-medium text-neutral-700 mb-4">نمودار فروش محصولات (۲۰ محصول برتر)</h3>
...
function ProductSalesTreemap({ rows }: { rows: Array<{ productTitle: string; totalRevenue: number; totalCount: number }> }) {
  ...
  const data = rows
    ...
    .map((row, index) => ({
      ...
      display: row.productTitle.length > 18 ? row.productTitle.slice(0, 18) + "…" : row.productTitle,
  ...
  const colors = ['#2563eb', '#7c3aed', '#06b6d4', '#f59e0b', '#10b981', '#ef4444', '#8b5cf6', '#84cc16'];
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-4 border border-neutral-200 rounded-lg shadow-lg" dir="rtl">
          <p className="font-medium text-neutral-800 mb-2">{data.name}</p>
          <div className="space-y-1 text-sm">
            <p className="text-blue-600">
              <span className="font-medium">درآمد:</span> {data.revenue.toLocaleString()} تومان
            </p>
            <p className="text-green-600">
              <span className="font-medium">تعداد:</span> {data.count.toLocaleString()} عدد
            </p>
  ...
  return (
    <div className="w-full" dir="rtl">
      <div className="bg-white rounded-xl border border-neutral-200 p-2 md:p-4">
      ...
      <div className="mt-4 flex flex-col md:flex-row md:items-center justify-between text-sm text-neutral-600 gap-2" dir="rtl">
        ...
        <div className="text-xs text-neutral-500">
          <span className="md:hidden">نمایش {Math.min(rows.length, 8)} محصول از {rows.length} محصول</span>
          <span className="hidden md:inline">نمایش {Math.min(rows.length, 20)} محصول از {rows.length} محصول</span>
        </div>
      </div>
    </div>
  );
