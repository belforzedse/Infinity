...
export default function PDPCommentListItem({ review, isReply = false, onStatusUpdate, onDelete }: Props) {
  ...
  return (
    <div className={`rounded-[20px] border border-slate-100 bg-[#FAFAF9] p-6 flex flex-col gap-[10px] ${isReply ? "mr-8 mt-4 bg-white shadow-sm" : "mb-4"}`}>
      {/* Review Header */}
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-[6px]">
            <div className="text-[18px] font-medium text-[#262626] leading-[1.238]">{displayName}</div>
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex items-center gap-3">
            {!isReply && (
              <button className="text-[#94A3B8] hover:text-[#DB2777] transition-colors">
                <ReplyIcon />
              </button>
            )}
            <div className="flex items-center gap-1 text-[#525252]">
              <span className="text-[12px] font-medium leading-[1]">{review.DislikeCounts || 0}</span>
              <button className="hover:text-red-500 transition-colors flex items-center justify-center">
                <ThumbsDownIcon />
              </button>
            </div>
            <div className="flex items-center gap-1 text-[#525252]">
              <span className="text-[12px] font-medium leading-[1]">{review.LikeCounts || 0}</span>
              <button className="hover:text-green-500 transition-colors flex items-center justify-center">
                <ThumbsUpIcon />
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-row-reverse gap-0.5">
            {[...Array(5)].map((_, i) => (
              i < review.Rate ? <StarIcon key={i} className="h-3.5 w-3.5" /> : <EmptyStarIcon key={i} className="h-3.5 w-3.5" />
            ))}
          </div>
      ...
      {/* Review Content */}
      <div className="text-[14px] leading-[1.68] text-[#525252]">
        {review.Content}
      </div>
