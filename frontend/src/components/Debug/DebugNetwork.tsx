    ...
    <div className="space-y-2">
      ...
      <div className="text-xs grid grid-cols-2 gap-2">
        <div className="rounded bg-gray-800 p-2">
          ...
          <div className="font-semibold text-white">{totalRequests}</div>
        </div>
        <div className="rounded bg-gray-800 p-2">
          <div className="text-gray-400">Avg Duration</div>
          <div className="font-semibold text-white">{avgDuration}ms</div>
        </div>
        <div className="rounded bg-gray-800 p-2">
          <div className="text-gray-400">Success</div>
          <div className="font-semibold text-green-400">{successfulRequests}</div>
        </div>
        <div className="rounded bg-gray-800 p-2">
          <div className="text-gray-400">Failed</div>
          <div className="font-semibold text-red-400">{failedRequests}</div>
        </div>
      </div>
        ...
        {filteredRequests.length === 0 ? (
          ...
          filteredRequests
            ...
            .map((request) => (
              <div
                key={request.id}
                className="rounded border-b border-gray-700 p-2 hover:bg-gray-800"
              >
                <div className="text-xs flex items-center justify-between">
                  <div className="flex min-w-0 flex-1 items-center space-x-2">
                    ...
                    <span className="flex-1 truncate text-gray-300" title={request.url}>
                      {request.url}
                    </span>
                  </div>
                ...
                {request.type && <div className="text-xs mt-1 text-gray-500">{request.type}</div>}
              </div>
            ))
        )}
      </div>
