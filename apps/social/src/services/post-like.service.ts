...
const POST_LIKE_TIMEOUT_MS = 15000;
...
function likedPostId(entry: unknown): string | null {
...
export async function getLikedPostIds(): Promise<Set<string>> {
  try {
    const response = await apiClient.get<unknown>(ENDPOINTS.POST_LIKES.USER_LIKES, {
      headers: authHeaders(),
      cache: "no-store",
      timeout: POST_LIKE_TIMEOUT_MS,
      retries: 0,
    });

    return new Set(readRows(response).map(likedPostId).filter((id): id is string => id != null));
  } catch (error: unknown) {
...
export async function togglePostLike(postId: string | number): Promise<TogglePostLikeResult> {
  try {
    const response = await apiClient.post<unknown>(
      ENDPOINTS.POST_LIKES.TOGGLE,
      { postId },
      {
        headers: authHeaders(),
        cache: "no-store",
