...
export const getHomeSectionProducts = async (
  ...
  if (assignment.mode === "manual") {
    return getProductsByIds(assignment.productIds);
  }
  ...
  try {
    ...
    return formatProductsToCardProps((response as { data?: unknown[] })?.data ?? []);
  } catch (error) {
    logger.error("[Homepage] getHomeSectionProducts error:", error as any);
    return [];
  }
};
