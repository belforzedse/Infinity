  ...
  return (
    <div className="space-y-3">
      ...
      <EditorSection title="محتوا">
        <SlotTextField label="عنوان کارت" value={slot.title} onChange={(value) => onChange({ ...slot, title: value })} editorField="title" />
        <SlotTextField label="زیرعنوان کارت" value={slot.subtitle} onChange={(value) => onChange({ ...slot, subtitle: value })} editorField="subtitle" />
        <SlotTextField label="متن دکمه" value={slot.buttonLabel} onChange={(value) => onChange({ ...slot, buttonLabel: value })} editorField="buttonLabel" />
      </EditorSection>

      <EditorSection title="لینک">
        <label className="text-xs text-slate-600">
          نوع لینک
          <select
      ...
      <EditorSection title="پس زمینه">
        <SlotTextField
          ...
          type="color"
          inputClassName="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white"
        />
      ...
      <EditorSection title="تصویر محصول">
        <div>
          <p className="mb-1 text-xs text-slate-600">تصویر کارت</p>
          <ImageUploadField value={slot.imageUrl} onChange={(value) => onChange({ ...slot, imageUrl: value })} />
        </div>

        <SlotTextField label="متن جایگزین تصویر" value={slot.imageAlt} onChange={(value) => onChange({ ...slot, imageAlt: value })} />
        ...
        <SizeRangeControl
          ...
          step={1}
          unit="%"
          onChange={(percent) => onChange({ ...slot, imageCustomWidth: percentToCustomWidth(percent) })}
        />
        ...
        <SizeRangeControl
          ...
          step={1}
          unit="%"
          onChange={(x) => onChange({ ...slot, imageObjectPosition: cardPercentsToObjectPosition(x, imagePosition.y) })}
        />
        ...
        <SizeRangeControl
          ...
          step={1}
          unit="%"
          onChange={(y) => onChange({ ...slot, imageObjectPosition: cardPercentsToObjectPosition(imagePosition.x, y) })}
        />
      </EditorSection>

      <EditorSection title="تایپوگرافی">
        <TextStyleEditor label="استایل عنوان" value={slot.titleStyle} onChange={(titleStyle) => onChange({ ...slot, titleStyle })} />
        <TextStyleEditor label="استایل متن دکمه" value={slot.buttonStyle} onChange={(buttonStyle) => onChange({ ...slot, buttonStyle })} />
      </EditorSection>
