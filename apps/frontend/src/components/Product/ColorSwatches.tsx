...
const ColorSwatches: React.FC<ColorSwatchesProps> = ({
  ...
  const remaining = totalCount > visibleSwatches.length ? totalCount - visibleSwatches.length : 0;
  ...
  return (
    <div
      ...
      role="img"
      aria-label={totalCount > 0 ? `${faNum(totalCount)} رنگ` : "پیش‌نمایش رنگ"}
    >
      <div
        className={clsx(
      ...
      {remaining > 0 && (
        <span className={clsx("font-medium text-white", isSmall ? "text-[10px]" : "text-xs")}>
          +{faNum(remaining)}
        </span>
      )}
    </div>
